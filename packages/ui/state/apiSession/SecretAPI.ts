...
import { deleteSecretFromMongoDB, getSecretsFromMongoDB, saveSecretsToMongoDB } from "./toziplyindex";
...
export class SecretAPI {
  static getSecret = async ({
    ...
    //ReplaceWithZiply
    const secret = await getSecretsFromMongoDB({ secretId:this.getKeyId(secretId, specialKey) });
  ...
  static getSecretArray = async ({
    ...
    // const secret = await store.get(secretId);
      ...
      //ReplaceWithZiply
    const secretObject = await getSecretsFromMongoDB({ secretId });
    const secret = secretObject.value;
    try {
  ...
  static saveSecret = async ({
   ...
   //ReplaceWithZiply
   await saveSecretsToMongoDB({ secretId:this.getKeyId(secretId, specialKey), secretValue })
  };
  ...
  static deleteSecret = async ({ secretId }: { secretId: string }) => {
    ...
    // }
     ...
     //ReplaceWithZiply
     const response = await deleteSecretFromMongoDB({ secretId });
  };
