...
import { getSessionFoldersFromMongoDB, saveSessionFoldersToMongoDB } from "./toziplyindex";
...
export class FolderAPI {
  static addSessionToFolder = async (
    ...
    //ReplaceWithZiply
    await saveSessionFoldersToMongoDB({ sessionFolders: newFolders });
  ...
  static getFolder = async (folderId: string) => {
    ...
    // return (folders || []).find((folder) => folder.id === folderId);
     //ReplaceWithZiply
     const folders: RecipeSessionFolder[] = await getSessionFoldersFromMongoDB();
     return (folders || []).find((folder) => folder.id === folderId);
  };
  ...
  static getAllFolders = async () => {
    ...
    if (hasChanged && migratedFolders) {
      //ReplaceWithZiply
      await saveSessionFoldersToMongoDB({ sessionFolders: migratedFolders });
    }
  ...
  static setFolders = async (folders: RecipeSessionFolder[]) => {
    ...
    //ReplaceWithZiply
    await saveSessionFoldersToMongoDB({ sessionFolders: folders });
    eventEmitter.emit("refreshFolders");
  };
