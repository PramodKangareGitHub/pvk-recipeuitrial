...
export interface APISessionParameters {
  editorBody: string;
  editorQuery: string;
...
export async function getSessionsFromStore() {
  ...
  //ReplaceWithZiply
  const sessions = await getSessionsFromMongoDB();
  return sessions;
}
...
export async function saveSessionToStore(sessions: RecipeSession[]) {
  ...
  //ReplaceWithZiply
  const response = await saveSessionsToMongoDB({ sessions: sessions });
...
export async function setParametersForSessionStore({
  ...
  // return store.put(parameters, session);
   //ReplaceWithZiply
   const postParameterResponse = await saveParametersToMongoDB({ session: session, parameters: parameters });
   return postParameterResponse
}
...
export async function deleteSession({
  ...
  const sessionsWithRecipeId = sessions.filter(
    (session:any) => session.recipeId === recipeId
  );
...
export async function deleteParametersForSessionStore({
  ...
  const response = await deleteParametersFromMongoDB({ session: session });
  return response;
}
...
export async function getParametersForSessionStore({
  ...
  // return store.get(session);
   ...
   //ReplaceWithZiply
   const parameter = await getParametersFromMongoDB({ session: session });
...
export async function setConfigForSessionStore({
  ...
  const postConfigResponse = await saveConfigToMongoDB({ recipeId: recipeId, config: config });
...
export async function getConfigForSessionStore({
  ...
  //ReplaceWithZiply
  const config = await getConfigFromMongoDB({ recipeId: recipeId });
...
export async function deleteConfigForSessionStore({
  ...
  if (sessionsWithRecipeId <= 1) {
    ...
    //ReplaceWithZiply
    await deleteConfigFromMongoDB({ recipeId: String(recipeId) });
...
import { deleteConfigFromMongoDB, deleteParametersFromMongoDB, getConfigFromMongoDB, getParametersFromMongoDB, getSessionsFromMongoDB, saveConfigToMongoDB, saveParametersToMongoDB, saveSessionsToMongoDB } from "./toziplyindex";
